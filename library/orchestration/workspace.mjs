...
function cmdAlloc(repo, args) {
  ...
  // 1. Per-worker database (template clone, instant).
  ...
  const envLocal = path.join(res.worktreePath, '.env.local');
  const overrides = [
    ...
    `REDIS_URL=${res.redisUrl}`,
    '',
  ].join('\n');
  fs.writeFileSync(envLocal, overrides, 'utf8');
  ...
  log(`  db         ${res.databaseUrl}`);
  log(`  redis      ${res.redisUrl}`);
  log('');
